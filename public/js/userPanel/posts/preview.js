...
import { baseUrl, calculateTimeDifference, getToken, getUrlParam, showSwal } from "../../../../utlis/utils.js";
...
window.addEventListener('load', async () => {
    ...
    let isEditMapOpened = false;
    let isPreviewMapOpened = false;
    ...
    const generateDatas = (data) => { 
        ...
        if (!isPreviewMapOpened) {
            let previewMap = L.map('preview-map').setView([data.map.x, data.map.y], 13);
            let marker = L.marker([data.map.x, data.map.y]).addTo(previewMap);
        ...
        editNavItem.addEventListener("click", () => {
            ...
            if (!isEditMapOpened) {
                isEditMapOpened = true
                let editMap = L.map('edit-map').setView([data.map.x, data.map.y], 13);
                ...
                let editMapMarker = L.marker([data.map.x, data.map.y], { icon: icon }).addTo(editMap);
