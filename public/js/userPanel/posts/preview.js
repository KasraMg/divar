...
window.addEventListener('load', async () => {
    ...
    const generateDatas = (data) => {
        ...
        if (!previewMap) {
            ...
            previewMapMarker = L.marker([data.map.x, data.map.y]).addTo(previewMap);
        } else {
    ...
    deletePostBtn.addEventListener('click', () => {
        ...
        showSwal('آیا  از حذف آگهی اطمینان دارید؟', "warning", ['خیر', 'بله'], (res) => {
            if (res) {
                loading.style.display = 'block'
                if (recentSeen) {
                    const newPosts = recentSeen.filter(post => post !== postId)
                    if (newPosts.length) {
                        saveIntoLocalStorage('recent-seen', [newPosts])
                    } else {
